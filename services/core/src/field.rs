...
impl Store {
    ...
    pub fn create_field(&mut self, input: &FieldInput) -> Result<Value> {
        ...
        let memory: Vec<_> = self
            ...
            .collect();
        let at = now();
        let body = json!({"schema_version":4,"input":input,"build":build,"state":"queued","created_at":at,
...
impl Store {
    pub fn field_duties(&self) -> Result<Vec<Value>> {
        ...
        for r in self.repositories()? {
            let c = &r["config"];
            duties.push(json!({"id":r["id"],"target":r["id"],"agent":"reviewer","enabled":c["enabled"]==true && c["removed"]!=true,"generation":c["generation"],"interval_seconds":c["interval_seconds"]}));
        }
        Ok(duties)
    }
