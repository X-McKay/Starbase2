...
func _initialize() -> void:
	...
	var entries: Array=JSON.parse_string(FileAccess.get_file_as_string(root+"/assets-production/characters/catalog.json"))
	for entry in entries:
		...
		var out: String=root+"/apps/world/assets/characters/"+id+"/"+id
		...
		var definition:="[gd_resource type=\"Resource\" load_steps=3 format=3]\n[ext_resource type=\"Script\" path=\"res://characters/definition.gd\" id=\"1\"]\n[ext_resource type=\"Texture2D\" path=\"res://assets/characters/%s/%s.png\" id=\"2\"]\n[resource]\nscript = ExtResource(\"1\")\nid = \"%s\"\natlas = ExtResource(\"2\")\ncanvas = Vector2i(%d,%d)\npivot = Vector2(%d,%d)\npixel_size = 0.011\nstride = %f\nsource = \"%s; %s\"\n" % [id,id,id,size.x,size.y,source.pivot[0],source.pivot[1],source.stride_m,source.source,source.tool]
