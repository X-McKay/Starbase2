...
const Catalog=preload("res://structure_catalog.gd")
...
func run() -> void:
	var ids := {}
	var dir := DirAccess.open("res://structures/definitions")
	for file in dir.get_files():
		if not file.ends_with(".tres"): continue
		var definition=load("res://structures/definitions/"+file)
	...
	invalid=load("res://structures/definitions/repair.tres").duplicate()
	invalid.exterior_image="res://assets/structures/engineering/engineering-hangar.png"
	invalid.exterior_scene="res://structures/exteriors/review.tscn"
	...
	for station in layout.get_children():
		...
		for local_rect in station.definition.collision_boxes:
			...
			for other in occupied:
				check(not footprint.grow(1.0).intersects(other.grow(1.0)),"Structures need at least a two-metre clear corridor between footprints")
	...
	for i in range(50):
		var definition=load("res://structures/definitions/repair.tres").duplicate()
		...
		definition.exterior_image="res://assets/structures/engineering/engineering-hangar.png"
		definition.yard_scene=""
		definition.seamless=false
		definition.interior_scene="res://structures/interiors/engineering-3d.tscn"
	...
	root.add_child(hud)
	hud.set_structures(group.get_children())
