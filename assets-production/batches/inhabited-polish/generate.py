...
LEDGER = ROOT / "meshy_output/inhabited-polish-ledger.json"
...
def summary(ledger):
    ...
    return {
        ...
        "remaining_authorized": 1000 - actual - pending,
        "stages": {
            key: {
                field: item[field]
...
def run(args):
    ...
    with LEDGER.with_suffix(".lock").open("a") as lock:
        try:
            ...
            raise SafeStop("Another inhabited-polish writer is active") from None
        ...
        if key not in stages:
            ...
            require(
                ...
                and plan.get("approved_credit_cap") == 1000
                and plan.get("prior_generation_credits") == 0,
                "This batch requires the separate approved 1000-credit cap",
            )
            require(
