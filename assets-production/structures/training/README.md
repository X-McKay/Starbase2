...
The selected scripted source is `blender/training-polished.blend`, with collision
and interaction composition in `layout.json`. Fitted exterior source is
`blender/training-hull.blend`, exported as
`apps/world/assets/structures/training/hull.glb`. The original
`blender/gym-building.blend` is the retained, unmodified baseline and is not
selected by this pipeline. Authored runtime media are `gym-shell.glb` and
`gym-interior.glb` in the same runtime directory. The connector composes them
in `apps/world/structures/exteriors/gym-continuous.tscn` and
`apps/world/structures/interiors/gym-continuous.tscn`.
...
Generated equipment dependencies: `simulation-station, resistance-bench`. Their preparation reads retained paid GLBs
...
/Applications/Blender.app/Contents/MacOS/Blender --background --factory-startup --python assets-production/scripts/build_remaining_structures.py -- --asset training
/Applications/Blender.app/Contents/MacOS/Blender --background --factory-startup --python assets-production/scripts/prepare_remaining_hulls.py -- --asset training
python3 assets-production/scripts/connect_remaining_structures.py --asset training
