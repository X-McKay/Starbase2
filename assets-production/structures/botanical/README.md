...
Paired cultivation beds, hydroponic racks and an authored water-service system. Room ID remains `greenhouse`. This is a scenery context with direct visits and no operational binding. Engineering and the current characters are preserved.
...
and a fitted whole generated hull around a **17 × 12 m** exact playable room.
...
The selected scripted source is `blender/botanical-polished.blend`, with collision
and interaction composition in `layout.json`. Fitted exterior source is
`blender/botanical-hull.blend`, exported as
`apps/world/assets/structures/botanical/hull.glb`. The original
`blender/greenhouse-building.blend` is the retained, unmodified baseline and is not
selected by this pipeline. Authored runtime media are `greenhouse-shell.glb` and
`greenhouse-interior.glb` in the same runtime directory. The connector composes them
in `apps/world/structures/exteriors/greenhouse-continuous.tscn` and
`apps/world/structures/interiors/greenhouse-continuous.tscn`.
...
Generated equipment dependencies: `hydroponic-rack, botany-lab-bench`. Their preparation reads retained paid GLBs
...
/Applications/Blender.app/Contents/MacOS/Blender --background --factory-startup --python assets-production/scripts/build_remaining_structures.py -- --asset botanical
/Applications/Blender.app/Contents/MacOS/Blender --background --factory-startup --python assets-production/scripts/prepare_remaining_hulls.py -- --asset botanical
python3 assets-production/scripts/connect_remaining_structures.py --asset botanical
...
and generous floor space remain visible art judgments. The glazing is opaque
textured foliage rather than true interior transparency; the initially unsupported water pipe was corrected with a bench connection
and vertical downleg. A nearby approach outcrop moved from (12, 35) to (10, 42)
after a route-check failure; the corrected full world suite passed. Native stills do not certify
