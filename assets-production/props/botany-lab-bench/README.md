...
qualification passed for local review. Owner art approval remains open. Research workbench complementing the cultivation beds and water services. This is static decorative equipment, without a rig or
...
editable source is `blender/botany-lab-bench.blend`; runtime is
`apps/world/assets/props/botany-lab-bench/botany-lab-bench.glb`. Preparation preserves imported
orientation and PBR materials, centers the floor origin and uniformly fits the
model inside **2.7 × 1 × 1.8 m** (width × depth × height). This is a maximum fit box,
...
```sh
/Applications/Blender.app/Contents/MacOS/Blender --background --factory-startup --python assets-production/scripts/prepare_remaining_props.py -- --asset botany-lab-bench
...
must exist before connection. These botanicals make no paid API calls. Preparation
requires the actual downloaded GLB referenced by the complete `botany-lab-bench/mesh`
