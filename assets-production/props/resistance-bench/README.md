...
qualification passed for local review. Owner art approval remains open. Exercise apparatus giving Training a distinct physical practice area. This is static decorative equipment, without a rig or
...
editable source is `blender/resistance-bench.blend`; runtime is
`apps/world/assets/props/resistance-bench/resistance-bench.glb`. Preparation preserves imported
orientation and PBR materials, centers the floor origin and uniformly fits the
model inside **2.4 × 1.2 × 2.6 m** (width × depth × height). This is a maximum fit box,
...
```sh
/Applications/Blender.app/Contents/MacOS/Blender --background --factory-startup --python assets-production/scripts/prepare_remaining_props.py -- --asset resistance-bench
...
must exist before connection. These trainings make no paid API calls. Preparation
requires the actual downloaded GLB referenced by the complete `resistance-bench/mesh`
