...
qualification passed for local review. Owner art approval remains open. Roof-mounted dish and sensor assembly; prepared mesh names use the Roof prefix for cutaway discovery. This is static decorative equipment, without a rig or
operational authority; animation or physical proximity cannot imply backend work.

Consumer: [command](../../structures/command/README.md). The selected
editable source is `blender/communications-array.blend`; runtime is
`apps/world/assets/props/communications-array/communications-array.glb`. Preparation preserves imported
orientation and PBR materials, centers the floor origin and uniformly fits the
model inside **3 × 2.4 × 2.8 m** (width × depth × height). This is a maximum fit box,
...
```sh
/Applications/Blender.app/Contents/MacOS/Blender --background --factory-startup --python assets-production/scripts/prepare_remaining_props.py -- --asset communications-array
...
requires the actual downloaded GLB referenced by the complete `communications-array/mesh`
