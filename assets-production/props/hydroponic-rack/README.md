...
qualification passed for local review. Owner art approval remains open. Cultivation apparatus instanced twice per bed, rotated 90 degrees about Godot Y and shifted rearward to clear the terminal. This is static decorative equipment, without a rig or
operational authority; animation or physical proximity cannot imply backend work.

Consumer: [botanical](../../structures/botanical/README.md). The selected
editable source is `blender/hydroponic-rack.blend`; runtime is
`apps/world/assets/props/hydroponic-rack/hydroponic-rack.glb`. Preparation preserves imported
orientation and PBR materials, centers the floor origin and uniformly fits the
model inside **2.2 × 1.25 × 2.2 m** (width × depth × height). This is a maximum fit box,
...
```sh
/Applications/Blender.app/Contents/MacOS/Blender --background --factory-startup --python assets-production/scripts/prepare_remaining_props.py -- --asset hydroponic-rack
...
must exist before connection. These commands make no paid API calls. Preparation
requires the actual downloaded GLB referenced by the complete `hydroponic-rack/mesh`
