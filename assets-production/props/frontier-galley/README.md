...
qualification passed for local review. Owner art approval remains open. Compact galley rotated 90 degrees about Godot Y for side-wall placement. This is static decorative equipment, without a rig or
...
editable source is `blender/frontier-galley.blend`; runtime is
`apps/world/assets/props/frontier-galley/frontier-galley.glb`. Preparation preserves imported
orientation and PBR materials, centers the floor origin and uniformly fits the
model inside **2.6 × 0.8 × 2.1 m** (width × depth × height). This is a maximum fit box,
...
```sh
/Applications/Blender.app/Contents/MacOS/Blender --background --factory-startup --python assets-production/scripts/prepare_remaining_props.py -- --asset frontier-galley
...
requires the actual downloaded GLB referenced by the complete `frontier-galley/mesh`
