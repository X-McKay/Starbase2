...
qualification passed for local review. Owner art approval remains open. One simulator model instanced twice within separately authored collision bounds. This is static decorative equipment, without a rig or
operational authority; animation or physical proximity cannot imply backend work.

Consumer: [training](../../structures/training/README.md). The selected
editable source is `blender/simulation-station.blend`; runtime is
`apps/world/assets/props/simulation-station/simulation-station.glb`. Preparation preserves imported
orientation and PBR materials, centers the floor origin and uniformly fits the
model inside **2.2 × 2.2 × 2.6 m** (width × depth × height). This is a maximum fit box,
...
```sh
/Applications/Blender.app/Contents/MacOS/Blender --background --factory-startup --python assets-production/scripts/prepare_remaining_props.py -- --asset simulation-station
...
requires the actual downloaded GLB referenced by the complete `simulation-station/mesh`
