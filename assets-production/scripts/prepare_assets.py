...
for item in batch["tasks"]:
    ...
    if item["asset"] == "engineering-exterior":
        ...
        for mat in list(bpy.data.materials):
            ...
            bpy.ops.object.join()
        meshes = [obj for obj in bpy.context.scene.objects if obj.type == "MESH"]
    family = "props/reactor-apparatus" if item["asset"] == "reactor-apparatus" else "structures/engineering-prototype"
    output = root / "apps/world/assets" / family / f"{item['asset']}.glb"
    output.parent.mkdir(parents=True, exist_ok=True)
    source_dir = root / "assets-production" / family / "blender"
    source_dir.mkdir(parents=True, exist_ok=True)
    bpy.ops.wm.save_as_mainfile(compress=True, filepath=str(source_dir / f"{item['asset']}.blend"))
...
(root / "assets-production/batches/meshy-blender/provenance.json").write_text(json.dumps(provenance, indent=2) + "\n")
