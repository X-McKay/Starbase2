...
layout = json.loads((root / "assets-production/structures/colony-layout.json").read_text())
...
for kind, data in layout.items():
    ...
    if kind == "repair":
        reactor = "res://assets/props/reactor-apparatus/reactor-apparatus.glb"
    ...
    (world / f"structures/interiors/{kind}-continuous.tscn").write_text(text)
    ...
    text += "seamless = true\n"
    text += f'exterior_scene = "res://structures/exteriors/{kind}-continuous.tscn"\n'
    text += f'interior_scene = "res://structures/interiors/{kind}-continuous.tscn"\n'
