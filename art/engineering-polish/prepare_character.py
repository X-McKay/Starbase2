...
art_dir = ROOT / ("art/cybercat-vanguard" if vanguard else "art/engineering-polish")
world_dir = ROOT / (
    "apps/world/art/cybercat-vanguard" if vanguard else "apps/world/art/engineering-polish"
)
...
bpy.ops.wm.save_as_mainfile(filepath=str(art_dir / (asset_name + "-imported.blend")))
...
for obj in meshes:
    obj.select_set(True)
bpy.context.view_layer.objects.active = rig
bpy.ops.wm.save_as_mainfile(filepath=str(art_dir / (asset_name + ".blend")))
